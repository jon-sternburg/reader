...
export default function Sidebar_New_Annotation(props: SNA_Props) {
  ...
  const [color, set_color] = useState<string>(
    props.edit !== null && props.edit.data.color
      ? props.edit.data.color
      : "#ffeb3b"
  );
  const [picked_category, set_picked_category] = useState<option_uc | null>(
    props.edit !== null && props.edit.data.category
      ? props.edit.data.category
      : null
  );
  ...
  useEffect(() => {
    ...
    if (props.edit !== null) {
      if (
        ...
        input_ref.current !== undefined
      ) {
        input_ref.current.value = props.edit.data.title;
      }
      if (
        textarea_ref &&
        textarea_ref.current !== null &&
        textarea_ref.current !== undefined
      ) {
  ...
  function handle_save(
    ...
    let ta_val =
      textarea_ref.current !== null ? textarea_ref.current.value : "";
    let input_val = input_ref.current !== null ? input_ref.current.value : "";

    props.handle_save_annotation(
      picked_category,
      color,
      props.edit,
      ta_val,
  ...
  return (
    <Fragment>
        ...
        <div className={styles.pick_color_wrap}>
          <div className={styles.relative_wrap}>
            ...
            <button
              ...
              type={"button"}
              className={styles.highlight_color_button}
              onClick={() => toggle_color_panel()}
            >
              <span>Highlight color</span>
              <div
                className={styles.picked_color_sample}
      ...
      <div className={styles.annotation_title_wrap}>
        <input
          ref={input_ref}
          className={styles.title_input_search}
          placeholder="Title..."
          name="annotation_title_search"
          type="text"
        />
      </div>
      <div className={styles.annotation_text_wrap_inner}>
        ...
        <div className={styles.button_wrap}>
          <div
            ...
            onClick={() => handle_save(picked_category, color, props.edit)}
          >
            Save
          </div>
          <div className={styles.cancel} onClick={() => handle_cancel()}>
            Cancel
          </div>
        </div>
      </div>
