...
export default function Page_User(props: PU_Props): JSX.Element {
  ...
  const [edit, set_edit] = useState<Edit_State>({
    show: false,
    annotation: null,
    book: null,
  });
  ...
  function handle_save_annotation(
    ...
    if (edit.book !== null && edit.annotation !== null && user_data !== null) {
      ...
      let matching_index = annotations.findIndex(
        (x) => x.cfiRange == annotation.cfiRange
      );
      ...
      if (picked_category !== null) {
        annotations[matching_index].data.category = picked_category;
      }
      let uc =
        picked_category == null ||
        edit.book.user_categories.filter(
          (x) => x.label.toLowerCase() == picked_category.label.toLowerCase()
        ).length > 0
          ? edit.book.user_categories
          : [...edit.book.user_categories, picked_category];
      set_book_data({
        id: edit.book.id,
        name: edit.book.name,
            ...
            <Fragment>
              ...
              <section className={styles.user_info_wrap}>
                {size.width >= 1000 && (
                  <nav className={styles.user_top_bar}>
                    ...
                    <button
                      aria-label={"Home"}
                      type={"button"}
                      className={styles.user_page_home_button}
                    >
                      <AiFillHome
                ...
                <section className={styles.user_book_list}>
                  ...
                  {user_data !== null ? (
                    <Fragment>
                      {user_data.books.length > 0 &&
                        props.user_data.books.map((x, i) => (
                          <Book_Item
                            key={i}
                            book_item={x}
                    ...
                    <div style={{ marginLeft: "10px" }}>
                      You don&apos;t have any saved annotations.
                    </div>
                  )}
                </section>
              </section>
...
function Book_Item(props: BI_Props) {
  ...
  let annotation_list =
    x.annotations.length > 1 ? x.annotations.slice(1) : x.annotations;
  let view_more_text =
    annotation_list.length == 1
      ? "View 1 more annotation"
      : `View ${x.annotations.length - 1} more annotations`;
  return (
    <article className={styles.book_item}>
      <header
        className={styles.book_item_title}
      ...
      <section className={styles.annotations_wrapper}>
        ...
        {show &&
          x.annotations.length > 1 &&
          annotation_list.map((y, i_) => (
            <Annotation
              key={i_}
              annotation={y}
              i={i_}
        ...
        {x.annotations.length > 1 && !show && (
          <div
            onClick={() => handle_toggle()}
            className={styles.show_more_annotations}
          >
            {view_more_text}
          </div>
        )}
  ...
  return (
    <Fragment>
        ...
        <div className={styles.delete_prompt_frame}>
          ...
          <div className={styles.bottom_buttons}>
            <button
              ...
              type={"button"}
              onClick={() => delete_annotation()}
            >
              Delete
            </button>
            <button
              aria-label={"Confirm cancel annotation"}
              type={"button"}
      ...
      <article className={styles.annotation_item}>
        <header>
          <h6>
            {title} - <span style={{ fontStyle: "italic" }}>{time}</span>
          </h6>
        </header>
        ...
        <footer className={styles.annotation_bottom_bar}>
          <button
            aria-label={"View annotation in book"}
            type={"button"}
            onClick={() => view_in_book()}
          >
            View
          </button>
          ...
          <button
            ...
            type={"button"}
            onClick={() => props.edit_annotation(y, book)}
            className={styles.edit_annotation}
          >
            {" "}
            <MdEdit className={styles.edit_annotation_icon} /> <span>Edit</span>
          </button>
          <button
            aria-label={"Delete annotation"}
            type={"button"}
            onClick={() => delete_annotation_pre(y)}
            className={styles.delete_annotation}
          >
            {" "}
            <IoMdTrash className={styles.delete_annotation_icon} />
            <span>Delete</span>
          </button>
        </footer>
      </article>
