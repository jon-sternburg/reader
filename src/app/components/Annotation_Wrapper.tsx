...
export default function Annotation_Wrapper(props: A_Props) {
  ...
  let category = props.sparknotes
    ? null
    : props.x.data.category
    ? props.x.data.category.label
    : null;

  let color = props.sparknotes
    ? "#2196f3"
    : props.x.data.color
    ? props.x.data.color
    : "green";
  ...
  return (
    <li
      ...
      className={
        props.selected
          ? styles["result_li"] + " " + styles["selected"]
          : styles["result_li"]
      }
    >
      <header>
        <h6
          className={
            props.selected ? styles.selected_title : styles.not_selected_title
          }
        >
          {title}{" "}
          {!props.sparknotes && (
      ...
      <Fragment>
        <blockquote style={{ border: `2px solid ${color}` }}>
          ...
          <p onClick={() => element_clicked()} style={{ fontStyle: "italic" }}>
            ...{preview}...
          </p>
        </blockquote>
        <Fragment>
          ...
          <div className={styles.annotation_bottom_bar}>
            <button
              ...
              type={"button"}
              className={styles.view_in_book}
              onClick={() => element_clicked()}
            >
              View
            </button>
            {!props.sparknotes ? (
              <Fragment>
                <button
                  ...
                  type={"button"}
                  onClick={() => props.edit_annotation(props.x)}
                  className={styles.edit_annotation}
                >
                  {" "}
                  <MdEdit className={styles.edit_annotation_icon} />{" "}
                  <span>Edit</span>
                </button>
                <button
                  aria-label={"Delete annotation"}
                  type={"button"}
                  onClick={() => props.delete_annotation_pre(cfiRange, props.i)}
              ...
              <button
                aria-label={"Edit annotation"}
                type={"button"}
                onClick={() => go_to_source()}
                className={styles.edit_annotation}
              >
                {" "}
                <BiLinkAlt className={styles.edit_annotation_icon} />{" "}
                <span>Source</span>
              </button>
            )}
          </div>
