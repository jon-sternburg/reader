...
export default function Top_Bar_Book(props: TBB_Props) {
  ...
  let title =
    props.selected_book == null ? "Reader" : props.selected_book.title;

  return (
    <header
      className={styles.top_bar_frame_book}
    ...
    >
      ...
      <div className={styles.header_wrap}>
        {props.results_length <= 0 && props.keyvalue.length > 2 && (
          <button aria-label={"Submit search"} type={"button"}>
            <AiOutlineSearch
              className={styles.quit_search_topbar_book}
              onClick={(e) => props.handle_text_submit(e)}
            />
          </button>
        )}
        {props.results_length > 0 && (
          <button aria-label={"Cancel search"} type={"button"}>
            <MdClose
              className={styles.quit_search_topbar_book}
              onClick={() => props.clear_input()}
            />
          </button>
        )}
        {props.w > 1000 && (
        ...
        <nav className={styles.book_buttons_top_wrap}>
          {!props.logged_in ? (
            ...
            <a href={"/user"} className={styles.user_tag_top}>
