...
<div className = 'title_wrap'>
    ...
    <div id = 'title' onClick = {() => this.props.select_book(null)}>{title}</div>
   </div>

{this.props.selected_book !== null && (
<div id = 'home_button'onClick = {() => this.props.select_book(null)}> <AiFillHome id = 'home' /></div> )}
...
<Fragment>
{this.props.selected_book == null ? 
<ul className = 'search_results_homepage'>
{this.state.results.map((x, i) => {
return <li key = {x + i} onClick = {() => this.props.select_book(x)}>
<span id = 'res_title'>{x.title}</span>,
<span id = 'res_author'> {x.author}</span>
</li>
})}
</ul>
: null }
...
{w > 1000 && (
<div className = 'topbar_search_wrap'>
