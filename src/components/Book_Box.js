...
annotation_cb = (cfi, loc) => {
  console.log('SAVE:', cfi)
this.setState({draft_cfi: cfi, sidebar: 'new_annotation', editing: false})
}
...
this.state.rendition.annotations.add('highlight', cfiRange, {text: text, data: 'notes go here', section: matching[0].label, loc: loc},  this.annotation_cb(cfiRange, loc))
this.popup_ref.current.style.visibility = 'hidden'
...
/*
let intro_div = document_.getElementsByClassName('pgmonospaced pgheader')[0]
const new_intro_div = document.createElement('text');
intro_div.parentNode.replaceChild(new_intro_div, intro_div)
*/
...
if (  e.path[0].id !== 'annotation' && e.path[0].id !== 'annotation_icon' &&
      e.path[0].id !== 'highlight' && e.path[0].id !== 'highlight_icon' ) 
...
<div  className = 'book_box_frame' ref = {this.frame_ref}  style = {{width: w <= 1000 ? w :  w - 80}} >

{this.state.sidebar !== null && w > 1000 && (<div className = 'book_tint' onClick = {() => this.set_sidebar(null)}/>)}
...
{this.state.loading && (  <div className = 'loading'><div className="lds-default"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div></div>)}

<div  id = 'area' ref = {this.book_ref}  className = 'book_box_inner_wrap'>
...
{w > 1000 && ( 
  <Fragment>
{this.state.sidebar == null && (<div className ='arrow_left_arrow_wrap' onClick = {(e) => this.previous_page(e)}><IoIosArrowBack id = 'left_arrow_icon' /></div>)}
<div className ='arrow_right_arrow_wrap'  onClick = {(e) => this.next_page(e)}><IoIosArrowForward id = 'right_arrow_icon' /></div>
...
{w <= 1000 && ( 
<Fragment>
<div className = 'bottom_bar_wrap'>
<IoIosArrowBack id = 'left_arrow_icon' onClick = {(e) => this.previous_page(e)} />

<AiFillHome id = 'home_mobile' onClick = {() => this.props.select_book(null)} />
<FaEllipsisV id = 'settings_mobile' onClick = {() => this.props.select_book('menu')} />
<IoIosArrowForward id = 'right_arrow_icon' onClick = {(e) => this.next_page(e)} />
</div>
</Fragment>
...
<div ref = {this.popup_ref} id = 'popup' onClick = {(e) => e.preventDefault()} >
<div ref = {this.annotation_ref} id ='annotation' >
<IoIosCreate  id = 'annotation_icon' />
Annotation
</div>
 <div ref = {this.highlight_ref} id='highlight' >
<FaHighlighter id = 'highlight_icon' />
Highlight
</div>
</div>
